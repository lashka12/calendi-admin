  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="hidden lg:block">
        <h1 className="text-2xl font-semibold theme-text-primary">{t('settings.title')}</h1>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {/* Desktop: Tabs sidebar */}
        <div className="hidden lg:block lg:col-span-1">
          <div className="theme-bg-secondary theme-border border rounded-xl p-2 sticky top-24">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              return (
                <button
                ...
                >
                  <Icon className={`w-5 h-5 flex-shrink-0 mt-0.5 ${
                    tab.comingSoon ? "theme-text-tertiary" : activeTab === tab.id ? "" : "theme-text-tertiary"
                  }`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-medium truncate">{tab.name}</p>
                      {tab.comingSoon && (
                        <span className="px-1.5 py-0.5 theme-bg-tertiary theme-text-secondary text-[10px] font-medium rounded">
                          {t('settings.data.comingSoon')}
                        </span>
                      )}
                    </div>
                    <p className={`text-xs mt-0.5 truncate ${
                      tab.comingSoon ? "theme-text-tertiary" : activeTab === tab.id ? "opacity-70" : "theme-text-secondary"
                    }`}>
                      {tab.description}
                    </p>
                  </div>
            ...
            <div className="mt-2 pt-2 border-t theme-border">
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:theme-bg-hover transition-all text-left"
              >
                <div className="w-8 h-8 rounded-lg theme-bg-tertiary flex items-center justify-center flex-shrink-0">
                  <LogOut className="w-4 h-4 theme-text-secondary" />
                </div>
                <span className="text-sm font-medium theme-text-secondary">{t('nav.signOut')}</span>
              </button>
            </div>
          </div>
        </div>
            ...
            {mobileListMode ? (
              ...
              >
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="w-full theme-bg-secondary rounded-2xl p-4 shadow-sm border theme-border active:theme-bg-tertiary transition-colors"
                  dir={isRTL ? 'rtl' : 'ltr'}
                >
                  <div className="flex items-center gap-4">
                    {/* Avatar */}
                    <div className="w-14 h-14 rounded-full selected-tab border border-white/10 flex items-center justify-center text-lg font-semibold">
                    ...
                    <div className="flex-1 text-start">
                      ...
                      <p className="text-sm theme-text-secondary">{settings.businessName || 'Your Business'}</p>
                    </div>
                ...
                <motion.div
                  ...
                  className="theme-bg-secondary rounded-2xl shadow-sm border theme-border overflow-hidden divide-y divide-theme"
                >
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-4 transition-colors ${
                          tab.comingSoon ? 'opacity-60' : 'active:theme-bg-tertiary'
                        }`}
                        dir={isRTL ? 'rtl' : 'ltr'}
                      >
                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
                          tab.comingSoon ? 'theme-bg-tertiary' : 'theme-bg-tertiary'
                        }`}>
                          <Icon className={`w-5 h-5 ${tab.comingSoon ? 'theme-text-tertiary' : 'theme-text-secondary'}`} strokeWidth={2} />
                        </div>
                        <div className="flex-1 text-start">
                          <div className="flex items-center gap-2">
                            <p className={`text-[15px] font-medium ${tab.comingSoon ? 'theme-text-secondary' : 'theme-text-primary'}`}>{tab.name}</p>
                            {tab.comingSoon && (
                              <span className="px-1.5 py-0.5 theme-bg-tertiary theme-text-secondary text-[10px] font-medium rounded">
                                {t('settings.data.comingSoon')}
                              </span>
                          ...
                          <p className={`text-xs mt-0.5 ${tab.comingSoon ? 'theme-text-tertiary' : 'theme-text-secondary'}`}>{tab.description}</p>
                        </div>
                        {!tab.comingSoon && (
                          <ChevronRight className={`w-5 h-5 theme-text-tertiary ${isRTL ? 'rotate-180' : ''}`} />
                        )}
                  ...
                  <div className="border-t theme-border mt-1">
                    <button
                      ...
                      className="w-full flex items-center gap-3 px-4 py-4 transition-colors active:theme-bg-active"
                      dir={isRTL ? 'rtl' : 'ltr'}
                    >
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 theme-bg-tertiary">
                        <LogOut className="w-5 h-5 theme-text-secondary" strokeWidth={2} />
                      </div>
                      <div className="flex-1 text-start">
                        <p className="text-[15px] font-medium theme-text-secondary">{t('nav.signOut')}</p>
                      </div>
                    </button>
                  </div>
                </motion.div>
                ...
                <div className="text-center pt-4 space-y-3">
                  <p className="text-xs theme-text-tertiary">Calendi Admin {t('settings.app.version')} 1.9</p>
                  <button
                    onClick={async () => {
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium theme-text-secondary theme-bg-tertiary rounded-xl hover:theme-bg-active active:scale-95 transition-all"
                  >
              ...
              >
                {/* Fixed header */}
                <div 
                  className="fixed top-0 left-0 right-0 z-30 theme-bg-primary/95 backdrop-blur-md" 
                ...
                >
                  <div className="flex items-center h-14 px-4">
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      className="w-9 h-9 flex items-center justify-center rounded-full hover:theme-bg-tertiary active:theme-bg-active transition-colors touch-manipulation"
                    >
                      <ArrowLeft className={`w-5 h-5 theme-text-secondary ${isRTL ? 'rotate-180' : ''}`} />
                    </motion.button>
              ...
              {activeTab === "profile" && (
                <div className="space-y-5">
                  ...
                  <div className="theme-bg-secondary theme-border border rounded-2xl p-6 shadow-sm">
                    <h3 className="text-base font-semibold theme-text-primary mb-1">{t('settings.profile.personalInfo')}</h3>
                    <p className="text-sm theme-text-secondary mb-6">{t('settings.profile.personalInfoDesc')}</p>

                    {loadingPersonal ? (
                      <div className="flex items-center justify-center py-8">
                        <div className="w-6 h-6 border-2 theme-border border-t-current rounded-full animate-spin" />
                      </div>
                    ) : (
                      <>
                        <div className="space-y-5">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                              ...
                              <input
                                ...
                                className="w-full px-4 py-2.5 border theme-border rounded-xl text-sm theme-bg-secondary theme-text-primary focus:outline-none focus:ring-2 focus:ring-current focus:border-transparent"
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <Mail className={`absolute top-1/2 -translate-y-1/2 w-5 h-5 theme-text-tertiary ${isRTL ? 'right-3' : 'left-3'}`} />
                              <input
                                type="email"
                                value={personalData.email}
                                onChange={(e) => setPersonalData({ ...personalData, email: e.target.value })}
                                className={`w-full py-2.5 border theme-border rounded-xl text-sm theme-bg-secondary theme-text-primary focus:outline-none focus:ring-2 focus:ring-current focus:border-transparent ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'}`}
                                placeholder={t('settings.profile.email')}
                              />
                            </div>
                            <p className="text-xs theme-text-secondary mt-1.5">
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <Phone className={`absolute top-1/2 -translate-y-1/2 w-5 h-5 theme-text-tertiary ${isRTL ? 'right-3' : 'left-3'}`} />
                              <input
                                type="tel"
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium theme-text-secondary theme-bg-tertiary rounded-lg hover:theme-bg-active transition-colors disabled:opacity-50"
                              >
                                {gettingLocation ? (
                                  <div className="w-3.5 h-3.5 border-2 theme-border border-t-transparent rounded-full animate-spin" />
                                ) : (
                            ...
                            <div className="relative">
                              <MapPin className={`absolute top-3 w-5 h-5 theme-text-tertiary ${isRTL ? 'right-3' : 'left-3'}`} />
                              <textarea
                                rows={2}
                                value={personalData.address}
                                onChange={(e) => setPersonalData({ ...personalData, address: e.target.value })}
                                className={`w-full py-2.5 border theme-border rounded-xl text-sm theme-bg-secondary theme-text-primary focus:outline-none focus:ring-2 focus:ring-current focus:border-transparent resize-none ${isRTL ? 'pr-10 pl-4' : 'pl-10 pr-4'}`}
                        ...
                        <div className="mt-6 pt-6 border-t theme-border flex items-center justify-between">
                          {personalSaved && (
                          ...
                          <button 
                            ...
                            className="px-6 py-2.5 btn-action text-sm font-medium rounded-full active:scale-[0.98] transition-all disabled:opacity-50"
                          >
                            {savingPersonal ? (
              ...
              {activeTab === "business" && (
                <div className="space-y-5">
                  <div className="theme-bg-secondary theme-border border rounded-2xl p-6 shadow-sm">
                    <h3 className="text-base font-semibold theme-text-primary mb-1">{t('settings.business.title')}</h3>
                    <p className="text-sm theme-text-secondary mb-6">{t('settings.business.titleDesc')}</p>
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          readOnly
                          className="w-full px-4 py-2.5 border theme-border rounded-xl text-sm theme-bg-tertiary theme-text-secondary"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            readOnly
                            className="w-24 px-4 py-2.5 border theme-border rounded-xl text-sm theme-bg-tertiary theme-text-secondary"
                          />
                          <span className="text-sm theme-text-secondary">{t('settings.business.minutes')}</span>
                        </div>
                      </div>
                      ...
                      {/* Show Prices */}
                      <div className="flex items-center justify-between p-4 theme-bg-tertiary rounded-xl" dir={isRTL ? 'rtl' : 'ltr'}>
                        <div>
                          <p className="text-sm font-medium theme-text-primary">{t('settings.business.showPrices')}</p>
              ...
              {activeTab === "notifications" && (
                <div className="space-y-6">
                  <div className="theme-bg-secondary theme-border border rounded-xl p-6">
                    <h3 className="text-base font-semibold theme-text-primary mb-1">{t('settings.notifications.channels')}</h3>
                    <p className="text-sm theme-text-secondary mb-6">{t('settings.notifications.channelsDesc')}</p>
                    ...
                    <div className="space-y-4">
                      {[
                        ...
                        return (
                          <div key={channel.key} dir={isRTL ? 'rtl' : 'ltr'} className="flex items-center justify-between p-4 border theme-border rounded-xl hover:theme-bg-hover transition-colors">
                            <div className="flex items-start gap-3 flex-1">
                              <div className="w-10 h-10 theme-bg-tertiary rounded-xl flex items-center justify-center flex-shrink-0">
                                <Icon className="w-5 h-5 theme-text-secondary" />
                              </div>
                              <div>
                                <p className="text-sm font-medium theme-text-primary">{channel.label}</p>
                                <p className="text-sm theme-text-secondary mt-0.5">{channel.description}</p>
                              </div>
                            </div>
                            <ToggleSwitch
                  ...
                  <div className="theme-bg-secondary theme-border border rounded-xl p-6">
                    <div className="flex items-start gap-3 mb-6" dir={isRTL ? 'rtl' : 'ltr'}>
                      ...
                      <div>
                        <h3 className="text-base font-semibold theme-text-primary">{t('settings.notifications.pushAlerts')}</h3>
                        <p className="text-sm theme-text-secondary mt-0.5">
                    ...
                    {pushLoading ? (
                      <div className="flex items-center justify-center py-8">
                        <div className="w-6 h-6 border-2 theme-border border-t-gray-600 rounded-full animate-spin" />
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <div className="flex items-center justify-between p-4 theme-bg-tertiary rounded-xl gap-4" dir={isRTL ? 'rtl' : 'ltr'}>
                          ...
                          <button
                            ...
                            className={`px-4 py-2 text-sm font-medium rounded-xl transition-colors disabled:opacity-50 flex-shrink-0 ${
                              pushEnabled
                                ? "theme-bg-tertiary theme-text-secondary hover:theme-bg-active"
                                : "btn-action"
                            }`}
                          >
                            {pushSaving ? (
                        ...
                        {pushEnabled && (
                          <div className="pt-3 border-t theme-border">
                            <button
                              ...
                              className="text-sm theme-text-secondary hover:theme-text-primary font-medium flex items-center gap-2"
                            >
                  ...
                  <div className="theme-bg-secondary theme-border border rounded-xl p-6">
                    <h3 className="text-base font-semibold theme-text-primary mb-1">{t('settings.notifications.types')}</h3>
                    <p className="text-sm theme-text-secondary mb-6">{t('settings.notifications.typesDesc')}</p>
                    ...
                    <div className="space-y-1">
                      {[
                        ...
                        <div key={type.key} dir={isRTL ? 'rtl' : 'ltr'} className="flex items-start justify-between py-4 px-4 hover:theme-bg-hover rounded-xl transition-colors gap-4">
                          <div className="flex-1">
                            <p className="text-sm font-medium theme-text-primary">{type.label}</p>
                            <p className="text-sm theme-text-secondary mt-0.5">{type.description}</p>
                          </div>
                          <ToggleSwitch
                  ...
                  <div className="theme-bg-secondary theme-border border rounded-2xl overflow-hidden">
                    {/* Header */}
                    <div className="p-5 border-b theme-border" dir={isRTL ? 'rtl' : 'ltr'}>
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-[#25D366]/10 rounded-xl flex items-center justify-center flex-shrink-0">
                          <MessageSquare className="w-5 h-5 text-[#25D366]" />
                        </div>
                        <div className="flex-1">
                          <h3 className="text-base font-semibold theme-text-primary">{t('settings.notifications.whatsappReminders')}</h3>
                          <p className="text-sm theme-text-secondary mt-0.5">
                      ...
                      <div>
                        ...
                        <div className="p-5 border-b theme-border" dir={isRTL ? 'rtl' : 'ltr'}>
                          <div className="flex items-center justify-between gap-4">
                            <div>
                              <p className="text-sm font-medium theme-text-primary">{t('settings.notifications.enableReminders')}</p>
                              <p className="text-sm theme-text-secondary mt-0.5">
                        ...
                        {dailyReminder.enabled && (
                          <div className="p-5 space-y-5 theme-bg-tertiary/50" dir={isRTL ? 'rtl' : 'ltr'}>
                            {/* Send Time */}
                        ...
                        {/* Save Button */}
                        <div className="p-5 theme-bg-secondary border-t theme-border flex items-center justify-end gap-3" dir={isRTL ? 'rtl' : 'ltr'}>
                          {reminderSaved && (
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-700 rounded-xl p-6 text-white">
                    ...
                    <div className="grid grid-cols-3 gap-4 pt-6 border-t border-gray-700">
                      <div>
                        <p className="text-xs theme-text-tertiary mb-1">{t('settings.billing.bookings')}</p>
                        <p className="text-lg font-semibold">324 / ∞</p>
                      </div>
                      <div>
                        <p className="text-xs theme-text-tertiary mb-1">{t('settings.billing.teamMembers')}</p>
                        <p className="text-lg font-semibold">3 / 10</p>
                      </div>
                      <div>
                        <p className="text-xs theme-text-tertiary mb-1">{t('settings.billing.storage')}</p>
                        <p className="text-lg font-semibold">2.4 / 50 GB</p>
                      </div>
                    </div>

                    <div className="mt-6 flex gap-3">
                      <button className="flex-1 px-4 py-2.5 theme-bg-secondary theme-text-primary text-sm font-medium rounded-xl hover:theme-bg-tertiary transition-colors">
                  ...
                  {/* Payment Method */}
                  <div className="theme-bg-secondary theme-border border rounded-xl p-6">
                    <h3 className="text-base font-semibold theme-text-primary mb-6">{t('settings.billing.paymentMethod')}</h3>

                    <div className="flex items-center justify-between p-4 border theme-border rounded-xl mb-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-medium theme-text-primary">•••• •••• •••• 4242</p>
                          <p className="text-sm theme-text-secondary mt-0.5">{t('settings.billing.expires')} 12/2025</p>
                        </div>
                      </div>
                    ...
                    <button className="text-sm theme-text-secondary hover:theme-text-primary font-medium">
                  ...
                  <div className="theme-bg-secondary theme-border border rounded-xl p-6">
                    <div className="flex items-center justify-between mb-6">
                      <div>
                        <h3 className="text-base font-semibold theme-text-primary">{t('settings.billing.billingHistory')}</h3>
                        <p className="text-sm theme-text-secondary mt-0.5">{t('settings.billing.billingHistoryDesc')}</p>
                      </div>
                      <button className="text-sm text-indigo-600 hover:text-indigo-700 font-medium">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        <div
                          key={i}
                          className="flex items-center justify-between p-4 border theme-border rounded-xl hover:theme-bg-hover transition-colors"
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="text-sm font-medium theme-text-primary">{invoice.date}</p>
                              <div className="flex items-center gap-2 mt-0.5">
                                <p className="text-xs theme-text-secondary">{invoice.invoice}</p>
                          ...
                          <div className="flex items-center gap-3">
                            <p className="text-sm font-semibold theme-text-primary">{invoice.amount}</p>
              ...
              {activeTab === "preferences" && (
                <div className="space-y-6">
                  <div className="theme-bg-secondary theme-border border rounded-xl p-6">
                    <h3 className="text-base font-semibold theme-text-primary mb-1">{t('settings.preferences.regional')}</h3>
                    <p className="text-sm theme-text-secondary mb-6">{t('settings.preferences.regionalDesc')}</p>
              ...
              {activeTab === "data" && (
                <div className="space-y-6">
                  <div className="theme-bg-secondary theme-border border rounded-xl p-6">
                    <h3 className="text-base font-semibold theme-text-primary mb-1">{t('settings.data.title')}</h3>
                    <p className="text-sm theme-text-secondary mb-6">{t('settings.data.titleDesc')}</p>

                    <div className="space-y-4">
                      <button dir={isRTL ? 'rtl' : 'ltr'} className="w-full flex items-center justify-between p-4 border theme-border rounded-xl hover:theme-bg-hover transition-colors text-start">
                        <div className="flex items-center gap-3">
                          <Download className="w-5 h-5 theme-text-secondary flex-shrink-0" />
                          <div>
                            <p className="text-sm font-medium theme-text-primary">{t('settings.data.exportData')}</p>
                            <p className="text-sm theme-text-secondary mt-0.5">{t('settings.data.exportDataDesc')}</p>
                          </div>
                        </div>
                        <span className="px-2.5 py-1 theme-bg-tertiary theme-text-secondary text-xs font-medium rounded-lg flex-shrink-0">
                          {t('settings.data.comingSoon')}
                        </span>
                      </button>
                      ...
                      <button dir={isRTL ? 'rtl' : 'ltr'} className="w-full flex items-center justify-between p-4 border theme-border rounded-xl hover:theme-bg-hover transition-colors text-start">
                        <div className="flex items-center gap-3">
                          <Shield className="w-5 h-5 theme-text-secondary flex-shrink-0" />
                          <div>
                            <p className="text-sm font-medium theme-text-primary">{t('settings.data.privacyPolicy')}</p>
                            <p className="text-sm theme-text-secondary mt-0.5">{t('settings.data.privacyPolicyDesc')}</p>
                          </div>
                        </div>
                        <ExternalLink className="w-4 h-4 theme-text-tertiary flex-shrink-0" />
                      </button>
                    </div>
                  </div>
                </div>
              ...
              {activeTab === "contact" && (
                <div className="space-y-6">
                  {/* Header */}
                  <div className="theme-bg-secondary theme-border border rounded-2xl p-6" dir={isRTL ? 'rtl' : 'ltr'}>
                    <div className="flex items-center gap-4 mb-2">
                      <div className="w-12 h-12 selected-tab rounded-xl flex items-center justify-center">
                        <HelpCircle className="w-6 h-6" />
                      </div>
                      <div>
                        <h3 className="text-lg font-semibold theme-text-primary">{t('settings.contact.title')}</h3>
                        <p className="text-sm theme-text-secondary">{t('settings.contact.subtitle')}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="theme-bg-secondary theme-border border rounded-2xl overflow-hidden">
                    ...
                    <a 
                      ...
                      className="flex items-center gap-4 p-4 hover:theme-bg-hover transition-colors border-b theme-border"
                      dir={isRTL ? 'rtl' : 'ltr'}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold theme-text-primary">{t('settings.contact.email')}</p>
                        <p className="text-sm theme-text-secondary">{t('settings.contact.emailDesc')}</p>
                      </div>
                      <ChevronRight className={`w-5 h-5 theme-text-tertiary flex-shrink-0 ${isRTL ? 'rotate-180' : ''}`} />
                    </a>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold theme-text-primary">{t('settings.contact.whatsapp')}</p>
                        <p className="text-sm theme-text-secondary">{t('settings.contact.whatsappDesc')}</p>
                      </div>
                      <ChevronRight className={`w-5 h-5 theme-text-tertiary flex-shrink-0 ${isRTL ? 'rotate-180' : ''}`} />
                    </a>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold theme-text-primary">{t('settings.contact.phone')}</p>
                        <p className="text-sm theme-text-secondary">{t('settings.contact.phoneDesc')}</p>
                      </div>
                      <ChevronRight className={`w-5 h-5 theme-text-tertiary flex-shrink-0 ${isRTL ? 'rotate-180' : ''}`} />
                    </a>
                    ...
                    <a 
                      href="#"
                      className="flex items-center gap-4 p-4 hover:theme-bg-hover transition-colors"
                      dir={isRTL ? 'rtl' : 'ltr'}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold theme-text-primary">{t('settings.contact.faq')}</p>
                        <p className="text-sm theme-text-secondary">{t('settings.contact.faqDesc')}</p>
                      </div>
                      <ChevronRight className={`w-5 h-5 theme-text-tertiary flex-shrink-0 ${isRTL ? 'rotate-180' : ''}`} />
                    </a>
                  </div>
                  ...
                  <div className="theme-bg-secondary theme-border border rounded-2xl p-6" dir={isRTL ? 'rtl' : 'ltr'}>
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-10 h-10 theme-bg-tertiary rounded-xl flex items-center justify-center flex-shrink-0">
                        <Send className="w-5 h-5 theme-text-secondary" />
                      </div>
                      <div>
                        <h3 className="text-base font-semibold theme-text-primary">{t('settings.contact.feedback')}</h3>
                        <p className="text-sm theme-text-secondary">{t('settings.contact.feedbackDesc')}</p>
                      </div>
                    </div>
                    <textarea
                      rows={4}
                      placeholder={t('settings.contact.feedbackPlaceholder')}
                      className="w-full px-4 py-3 border theme-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-gray-800 focus:border-transparent resize-none"
                    />
